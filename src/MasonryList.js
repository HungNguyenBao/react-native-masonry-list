...
export default class MasonryList extends React.PureComponent {
  ...
  _getCalculatedDimensions(
    ...
    const newWidth =
      countDecimals(tempWidth) > 10
        ? parseFloat(tempWidth.toFixed(10))
        : tempWidth;
    const newHeight =
      countDecimals(tempHeight) > 10
  ...
  resolveImages(
    ...
    function _assignColumns(image, nColumns) {
      ...
      if (!columnHeightTotals[columnCounting - 1]) {
        ...
        columnHeightTotals[columnCounting - 1] =
          columnHeightTotals[columnCounting - 1] + height;
      }
          ...
          if (resolveTask && resolveTask.fork) {
            resolveTask.fork(
              ...
              resolvedData => {
                ...
                let finalizedData = setItemSource(
                  resolvedData,
                  itemSource,
                  resolvedImage
                );
                ...
                if (this.props.onImageResolved) {
                  finalizedData =
                    this.props.onImageResolved(finalizedData, renderIndex) ||
                    finalizedData;
                }

                if (renderIndex !== 0) {
                  this.setState(state => {
                    const sortedData = insertIntoColumn(
                      finalizedData,
                      state._sortedData,
                      sorted
                    );
          ...
          if (resolveTask && resolveTask.fork) {
            resolveTask.fork(
              ...
              resolvedImage => {
                ...
                if (this.props.onImageResolved) {
                  resolvedImage =
                    this.props.onImageResolved(resolvedImage, renderIndex) ||
                    resolvedImage;
                }
      ...
      <ScrollView>
        ...
        <FlatList
          style={{
            flex: 1,
            padding:
              ((this.props.layoutDimensions.width / 100) * this.props.spacing) /
              2,
